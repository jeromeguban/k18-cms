...
class Posting extends Model
{
    ...
    */
    ...
    public function scopeWhereQueryScopes($query)
    {
        ...
        if(request()->category){
            $query->where('postings.category', request()->category);
        }

        
        if(request()->category == 'Retail' && request()->store){
        ...
        if(request()->from && request()->to){
            $query->whereBetween('postings.created_at',[request()->from, request()->to]);
        }


        // Auction Posting Condition (Auction Show Module)
