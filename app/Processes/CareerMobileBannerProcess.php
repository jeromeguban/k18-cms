...
class CareerMobileBannerProcess
{
	protected $file;
	protected $posting;
	protected $filename;
    ...
    */
    ...
    public function uploadImage()
    {
        ...
        $location = public_path('images/careers/mobile/'.date('/Y/m/').$this->filename);
        $location_path = public_path('images/careers/mobile'.date('/Y/m/'));
    ...
    {
        // if(! Storage::disk('careers')->exists(date('/Y/m/').$this->filename)) {
        //     throw new \Exception('File is not uploaded');
        // }


        $this->career->update([
            'mobile_banner'  => '/images/careers/mobile'.date('/Y/m/').$this->filename
        ]);
