...
class PaymayaCallbackProcess
{
    ...
    public function __construct($request = null)
    {
        ...
        $validate_reference_code = explode('-', $this->request->requestReferenceNumber);
    ...
    */
    ...
    private function createPayment($order)
    {
       ...
       $payment_transaction =  PaymentTransaction::where('reference_code', $this->request->requestReferenceNumber)->first();
        ...
        $this->payment = Payment::create([
            ...
            'remarks'               => $this->payment_type ? "Payment via {$this->payment_type->name}" : "Payment via Paymaya", 
        ...
        switch($this->type) {
            ...
            case 'order':
                ...
                $this->orders = Order::selectedFields()
                                    ->joinOrderPosting()
                                    ->whereNotYetPaid()
                                    ->where('orders.reference_code', $this->request->requestReferenceNumber)
                                    ->groupBy([
                                        'orders.id'
                                    ])
                                    ->get();
            ...
            case 'bidder_deposit': 
                ...
                $this->bidder_deposit = BidderDeposit::where('reference_code', $this->request->requestReferenceNumber)
    ...
    {
        PaymentTransaction::where('reference_code', $this->request->requestReferenceNumber)
            ->update([ 
                'transaction_status'    => $this->request->status,
                // 'extras'                => str_replace("\\", "", json_encode($this->request->extras))
                'transaction_response'  => json_encode($this->request)
            ]);
