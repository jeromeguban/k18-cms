				...
				<div class="flex gap-x-6">
					...
					<div class="flex flex-col pb-3 gap-y-3 border-r border-gray-200">
						<div class="flex leading-loose">
							...
							<div class="w-96 pr-3">{{ user.first_name }}</div>
						</div>
						<div class="flex leading-loose">
							<div class="w-52 font-semibold pr-3">Last Name</div>
							<div class="w-96 pr-3">{{ user.last_name }}</div>
						</div>
						<div class="flex leading-loose">
							<div class="w-52 font-semibold pr-3">Phone</div>
							<div class="w-96 pr-3">{{ user.phone }}</div>
						</div>
						<div class="flex leading-loose">
							<div class="w-52 font-semibold pr-3">Email</div>
							<div class="w-96 pr-3">{{ user.email }}</div>
						</div>
						<div class="flex leading-loose">
							<div class="w-52 font-semibold pr-3">Created Date</div>
							<div class="w-96 pr-3">{{ user.created_at }}</div>
						</div>
						<div class="flex leading-loose">
							<div class="w-52 font-semibold pr-3">Modified Date</div>
							<div class="w-96 pr-3">{{ user.updated_at }}</div>
						</div>
					</div>
					<!-- 2nd column example -->
