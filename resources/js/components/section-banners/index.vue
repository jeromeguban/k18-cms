			...
			<div class="bg-white rounded-sm p-5 table-box-shadow">
				...
				<div class="w-full">
					...
					<div class="show-content__row">
									<file-upload
										method="POST"
										:url="'/section/'+$route.params.id+'/banners'"/>
								</div>
				</div>
			</div>
...
<script>
	export default {
		...
		methods: {
			submit() {
				this.form.post(`/section-banners`)
				.then(()=>{
				});
			},
			getBanners() {
				axios.get('/section/' +this.$route.params.id+ '/banners')
				.then(({data})=>{
					this.images = data;
				})
				.catch();
			...
			deleteBanner(id) {
				...
				.then(confirmed => {
					axios.delete(`/section/${this.section.id}/banners/${id}`)
					.then(()=>{
				        this.$modal.success({
