<template>
	<div ref="close" id="events-edit" class="modal" tabindex="-1" aria-hidden="true">
		<div class="modal-dialog modal-xl">
			<div class="modal-content">
				<!-- BEGIN: Modal Header -->
				<div class="modal-header bg-primary-1 text-theme-2">
					<h2 class="font-medium text-base mr-auto">Edit Event</h2>

				</div>
				<form class="">
					<div class="modal-body p-10">
						<div class="input-form">
							<label class="form-label w-full flex flex-col mt-2 sm:flex-row">
								Event Name
							...
							<label class="form-label w-full flex flex-col mt-2 mt-2 sm:flex-row">
								Event Description
								<span class="sm:ml-auto mt-1 sm:mt-0 text-xs text-theme-6 "
							...
							<label class="form-label w-full flex flex-col mt-3 sm:flex-row">
								Starting Time
								<span class="sm:ml-auto mt-1 sm:mt-0 text-xs text-theme-6 "
									v-if="form.errors.get('starting_time')" v-html="form.errors.get('starting_time')" />
							</label>
							<date-picker  value-type="format" format="YYYY-MM-DD HH:mm:ss" v-model="starting_time" type="datetime" class="w-full"></date-picker>
							...
							<multiselect v-model="term" :options="terms" name="term" :custom-label="getTermLabel"
							...
							<!-- <label for="validation-form-2" class="form-label w-full flex flex-col mt-2 sm:flex-row mt-3"  @change="changeRestriction">
	                                    Restriction
	                                </label>
	                                <div class="w-24 mt-1">
	                                    <label for="settings" class="cursor-pointer">
	                                        <input type="checkbox"  class="show-code form-check-switch mr-0 ml-3" :checked="form.restriction" @change="changeRestriction">
	                                    </label>
	                                </div> -->
							...
							<div v-if="form.type == 'Private'">
								...
								<date-picker  value-type="format" format="YYYY/MM/DD" v-model="form.access_request_info.from" type="datetime" class="w-full"></date-picker>
								...
								<date-picker  value-type="format" format="YYYY/MM/DD" v-model="form.access_request_info.to" type="datetime" class="w-full"></date-picker>
								...
								<div class="sm:mt-6 mt-0">
									...
									<div class="flex w-full" v-for="(valid_domain, index) in form.valid_domains">
...
export default {
	...
	created() {
		...
		this.getTerms();
		if(this.event.access_request_info){
			this.form.access_request_info = this.event.access_request_info ? JSON.parse(this.event.access_request_info) : null;
		}
		if(this.event.valid_domains){
			this.form.valid_domains = this.event.valid_domains ? JSON.parse(this.event.valid_domains) : null;
		}
		if(this.event.starting_time){
			this.starting_time = moment(this.event.starting_time).format("YYYY-MM-DD HH:mm:ss");
			this.form.starting_time = moment(this.event.starting_time).format("YYYY-MM-DD HH:mm:ss");
		}
	...
	watch:{
		'event'() {
			this.form = new Form(this.event);
			if(this.event.access_request_info){
				this.form.access_request_info = JSON.parse(this.event.access_request_info);
			}
			if(this.event.valid_domains) {
				this.form.valid_domains = JSON.parse(this.event.valid_domains);
			}
			if(this.event.starting_time) {
				this.starting_time = moment(this.event.starting_time).format("YYYY-MM-DD HH:mm:ss");
				this.form.starting_time = moment(this.event.starting_time).format("YYYY-MM-DD HH:mm:ss");
		...
		'form.access_request.from'() {
			this.form.access_request_info.from = moment(this.form.access_request_info.from).format('YYYY/MM/DD') ? moment(this.form.access_request_info.from).format('YYYY/MM/DD') : null;
		},

		'form.access_request.to'() {
			this.form.access_request_info.to = moment(this.form.access_request_info.to).format('YYYY/MM/DD') ? moment(this.form.access_request_info.to).format('YYYY/MM/DD') : null;
		},
	...
	methods: {
		submit() {
			this.isLoading = true;
			this.form.patch(`/events/${this.event.event_id}`)
			.then(() => {
			 this.isLoading = false;
		     this.$snotify.async('Please wait', 'Processing...', () => new Promise((resolve, reject) => {
				setTimeout(() => resolve({
					title: 'Success!',
					body: 'Event Successfuly Updated!',
					config: {
					timeout: 2000,
		...
		deleteRow(valid_domain_index) {
				this.form.valid_domains.splice(valid_domain_index, 1)
