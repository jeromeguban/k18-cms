...
<div>
	<div class="intro-y box p-5 mt-5">
        ...
        <div class="text-lg text-black font-semibold leading-loose">Show Details - Inquiry</div>
        <p class="text-gray-700 leading-loose border-b border-gray-200 pb-2 mb-4">This page contains information about your inquiry details</p>
            ...
            <div class="flex gap-x-6">
                ...
                <div class="flex flex-col pb-3 gap-y-3 border-r border-gray-200">
                    <div class="flex leading-loose">
                        ...
                        <div class="w-96 pr-3">{{ inquiry.full_name }}</div>
                    </div>
                    <div class="flex leading-loose">
                        <div class="w-52 font-semibold pr-3">Address</div>
                        <div class="w-96 pr-3">{{ inquiry.address }}</div>
                    </div>
                    <div class="flex leading-loose">
                        <div class="w-52 font-semibold pr-3">Email</div>
                        <div class="w-96 pr-3">{{ inquiry.email }}</div>
                    </div>
                    <div class="flex leading-loose">
                        <div class="w-52 font-semibold pr-3">Mobile no.</div>
                        <div class="w-96 pr-3">{{ inquiry.mobile_no }}</div>
                    </div>
					<div class="flex leading-loose">
                        <div class="w-52 font-semibold pr-3">Subject</div>
                        <div class="w-96 pr-3">{{ inquiry.subject }}</div>
                    </div>
					<div class="flex leading-loose">
                        <div class="w-52 font-semibold pr-3">Message</div>
                        <div class="w-96 pr-3">{{ inquiry.message }}</div>
                    </div>
                </div>
                <!-- 2nd column example -->
...
<script>
	export default {
		...
		methods: {
			show() {
				   axios.get('/inquiries/' + this.$route.params.id)
			        .then((response) => {
