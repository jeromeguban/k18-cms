<template>
    <div>
        <h2 class="intro-y text-lg font-medium mt-10"> Key Visuals </h2>
        ...
        <div class="intro-y box overflow-hidden mt-5">
            <div class="overflow-x-auto mt-4">
                <div class="box rounded-sm p-5 table-box-shadow">
                    <div class="flex flex-col ">
                        <div class="flex items-center mb-12">
                            <div class="flex lg:w-full justify-end md:w-full justify-end sm:w-32 justify-start">
                                <a href="" @click.prevent="" data-toggle="modal" data-target="#key-visual-create"
                        ...
                        <div class="flex">
                            <div class="lg:w-full md:w-full sm:w-32">
                                <draggable v-model="key_visuals" @change="updateSequenced"
                                    ...
                                    <div class="border mb-4 border-gray-300 border-solid shadow-md rounded w-full "
                                        v-for="(key_visual, index) in key_visuals" :key="key_visual.id">
                                        <div class="flex relative">
                                            ...
                                            <div class="flex p-12 w-full">
                                                <div class="flex w-full ml-6">
                                                    ...
                                                    <img alt="Banner" :src="key_visual.banner" data-action="zoom"
                                                        :style="'width: 120px; height: 50px;'"></img>

                                                    <div class="w-full flex flex-col justify-center ml-12">
                                                        <span class="text-lg">{{ key_visual.name }}</span>
                                                        <span
                                                            class="text-theme-1 underline hover:no-underline cursor-pointer mt-2">{{
                                                                    key_visual.link
                                                            }}</span>
                                                    ...
                                                    <div class="flex items-center w-24 ml-4">
                                                        <a class="flex text-theme-1" href="" data-toggle="modal"
                                                            data-target="#key-visual-edit"
                                                            @click.prevent="setKeyVisual(key_visual)">
                                                            <svg xmlns="http://www.w3.org/2000/svg" width="24"
                                                                height="24" viewBox="0 0 24 24" fill="none"
                                                                stroke="currentColor" stroke-width="1.5"
                                                                stroke-linecap="round" stroke-linejoin="round"
                                                                class="feather feather-edit w-4 h-4 mr-1 text-gray-600 mr-2">
                                                                <path
                                                        ...
                                                        <div class="w-24 mt-1">
                                                            <label for="settings" class="cursor-pointer">
                                                                <input type="checkbox"
                                                                    ...
                                                                    :id="'enable-' + index"
                                                                    :checked="key_visual.active == 1"
                                                                    @change="changeKeyVisualStatus(key_visual.id, index)">
                                                                <label :for="'enable-' + index"
                                                                    class="font-semibold text-gray-600"
                                                                    v-if="key_visual.active == 1">test</label>
                                                                <label :for="'enable-' + index"
                                            ...
                                            <a @click.prevent="deleteKeyVisual(key_visual.id)"
                                                class=" bg-theme-1 hover:bg-theme-6 w-16 flex items-center justify-center text-white cursor-pointer">
                    ...
                    <div v-if="key_visuals.length < 1" class="h-full flex items-center">
                    ...
                    <create @created="getKeyVisuals()"></create>
                    <edit @updated="getKeyVisuals()" :keyVisual="key_visual"></edit>
...
export default {
    ...
    computed: {
        'keyVisualSequencedIds'() {
            return this.key_visuals.map((key_visual) => {
                return key_visual.id
            })
        }
    },
    created() {
    ...
    methods: {
        getKeyVisuals() {
            axios.get('key-visuals')
                .then(({
                    data
                }) => {
                    this.key_visuals = data;
                }).catch({});
        },

        updateSequenced() {
            axios.post(`key-visuals/sequence`, {
                sequence: this.keyVisualSequencedIds,
            }).then(() => {
        ...
        changeKeyVisualStatus(id, index) {
            this.key_visuals[index].active = !this.key_visuals[index].active;
            axios.patch(`key-visuals/${id}/status`, {
                active: this.key_visuals[index].active,
            }).then(({
                data
            }) => { })
        ...
        deleteKeyVisual(id) {
            this.$modal.dialog({
                message: 'Are you sure you want to delete this key visual?',
                confirmButton: 'Okay',
                cancelButton: 'Cancel',
                title: 'Delete'
            })
                .then(confirmed => {
                    axios.delete(`key-visuals/${id}`)
                        .then(() => {
                            this.$modal.success({
                                message: 'You successfully deleted a key visual',
                                title: 'Deleted'
                            });
                            // Reload page
                            app.$emit('reload');
        ...
        setKeyVisual(key_visual) {
            this.key_visual = key_visual;
        }
    }
}
</script>
