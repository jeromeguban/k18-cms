...
<template>
	<div ref="close" id="key-visual-edit" class="modal" tabindex="-1" aria-hidden="true">
		<div class="modal-dialog modal-xl">
			<div class="modal-content">

				<!-- BEGIN: Modal Header -->
				<div class="modal-header bg-primary-1 text-theme-2">
					<h2 class="font-medium text-base mr-auto">Edit Key Visual</h2>
...
export default {
	...
	created() {
		this.form = new Form(this.keyVisual);
	},
    watch:{
		'keyVisual' () {
		this.form = new Form(this.keyVisual);
	...
	methods: {
		submit() {
			this.isLoading = true;
			this.form.patch(`key-visuals/${this.keyVisual.id}`)
			.then(()=>{
				 this.isLoading = false;
                 this.$snotify.async('Please wait', 'Processing...', () => new Promise((resolve, reject) => {
					setTimeout(() => resolve({
						title: 'Success!',
						body: 'Key Visual successfully updated!',
						config: {
						timeout: 2000,
