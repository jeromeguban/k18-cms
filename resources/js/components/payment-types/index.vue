<template>
	<div class="w-full">
		<table-template link="/payment-types" :fields="fields" modalIdentifier="#payment_types-create">
		<template slot="label">
        ...
        <template slot="status" slot-scope="props">
			<div class="flex items-center w-24 mr-6">
				<label v-if="props.item.status == 1" class="w-20 text-left text-md text-theme-9">Active</label>
				<label v-if="props.item.status == 0" class="w-20 text-left text-md text-theme-6">Inactive</label>
				<div class="w-24 mt-1">
					<label for="settings" class="cursor-pointer">
						<input type="checkbox"  class="show-code form-check-switch mr-0 ml-3" :id="'enable-'+index" :checked="props.item.status == 1" @change="changeStatus(props.item.id, props.item.status)">
						<label :for="'enable-'+index" class="font-semibold text-gray-600" v-if="props.item.status == 1"></label>
						<label :for="'enable-'+index" class="font-semibold text-gray-600" v-else></label>
					</label>
				</div>
			</div>
	    ...
	    <template slot="id" slot-scope="props">
	    	...
	    	<router-link :to="'/payment-types/' + props.item.id + '?tag=settings'" v-can="'view.payment-type'">
			...
			<a class="flex items-center mr-3" href="javascript:;" data-toggle="modal" data-target="#payment_types-edit"  @click.prevent="setPaymentType(props.item, props.index)" v-can="'update.payment-type'" >
			...
			<a class="flex items-center text-theme-6" href="javascript:;"  @click.prevent="deletePaymentType(props.item.id)" v-can="'delete.payment-type'">
	  	...
	  	<edit v-if="payment_type"
		  @update="clearPaymentType"
...
export default {
	...
	methods: {
		deletePaymentType(id) {
			...
			.then(confirmed => {
				axios.delete(`/payment-types/${id}`)
				.then(()=>{
			        this.$modal.success({
					 	message: 'You successfully deleted a Payment Type',
					 	title: 'Deleted'
					 });
					 // Reload page
				app.$emit('reload');
		...
		setPaymentType(payment_type, index) {
			console.log(index);
        ...
        changeActiveStatus(id,active) {
			...
			axios.patch(`payment-types/${id}/active`,{
			    active: active ,
			}).then(({data})=>{
        ...
        changeStatus(id,status) {
            ...
            axios.patch(`payment-types/${id}/status`,{
                status: status ,
            }).then(({data})=>{
