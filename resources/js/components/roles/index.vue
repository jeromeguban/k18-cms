<template>
	<div class="w-full">
		<table-template link="/roles" :fields="fields" modalIdentifier="#roles-create">
		<template slot="label">
			<h4>List of Roles</h4>
		</template>
	    ...
	    <template slot="id" slot-scope="props">
	    	...
	    	<router-link :to="'/roles/' + props.item.id + '?tag=acl'" v-can="'view.role'">
			...
			<a class="flex items-center mr-3" href="javascript:;" data-toggle="modal" data-target="#roles-edit"  @click.prevent="setRole(props.item, props.index)" v-can="'update.role'" >
			...
			<a class="flex items-center text-theme-6" href="javascript:;"  @click.prevent="deleteRole(props.item.id)" v-can="'delete.role'">
	...
	methods: {
		deleteRole(id) {
			...
			.then(confirmed => {  
				...
				.then(()=>{
			        this.$modal.success({
					 	message: 'You successfully deleted a role',
					 	title: 'Deleted'
					 });
					 // Reload page
				app.$emit('reload');
