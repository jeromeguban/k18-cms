        ...
        <form class="flex flex-col h-full" enctype="multipart/form-data">
            <div class="h-80 p-8 overflow-auto sideform-container--height">
                <div class="flex flex-col w-full mb-3 pr-1">
                    <label class="text-2xs font-semibold">First Name</label>
                    <input type="text" class="border border-solid border-gray-300 focus:border-gray-400 px-2 py-2 rounded outline-none w-full flex-0 h-10 mt-1" v-model="profile.first_name">
                    <span class="text-red-500 my-2 flex items-center text-2xs" v-if="form.errors.get('first_name')" v-html="form.errors.get('first_name')">
                    </span>
                </div>

                <div class="flex flex-col w-full mb-3 pr-1">
                    <label class="text-2xs font-semibold">Last Name</label>
                    <input type="text" class="border border-solid border-gray-300 focus:border-gray-400 px-2 py-2 rounded outline-none w-full flex-0 h-10 mt-1" v-model="profile.last_name">
                    <span class="text-red-500 my-2 flex items-center text-2xs" v-if="form.errors.get('last_name')" v-html="form.errors.get('last_name')">
                    </span>
                </div>

                <div class="flex flex-col w-full mb-3 pr-1">
                    <label class="text-2xs font-semibold">Phone</label>
                    <input type="text" class="border border-solid border-gray-300 focus:border-gray-400 px-2 py-2 rounded outline-none w-full flex-0 h-10 mt-1" v-model="profile.phone">
                    <span class="text-red-500 my-2 flex items-center text-2xs" v-if="form.errors.get('phone')" v-html="form.errors.get('phone')">
                    </span>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            submit() {
                this.isLoading = true;
                this.form.put(`/settings/profiles/${this.user}`)
                .then(()=>{
