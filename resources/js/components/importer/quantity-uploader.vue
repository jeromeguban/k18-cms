...
<div class="intro-y w-full h-full mb-2">
		<div class="flex flex-col mt-6 lg:flex-row">
				...
				<span class="text-center mb-4 py-2 uppercase text-white bg-theme-1 italic">Quantity Uploader</span>
				<div>
					<div class="intro-y mx-auto text-center">
                        <p v-if="loading">Please wait......</p>                         
                        <form data-single="true" action="/file-upload" class="mt-4">
                            <div class="fallback">
                                <input type="file" name="excel" @change="uploadQuantity"/>
                            </div>
...
<script>
	export default {
		...
		methods: {
			uploadQuantity(event) {
				this.form.file = event.target.files[0];
				this.loading = true;
				this.form.post('/quantity-upload')
				.then(response => {
