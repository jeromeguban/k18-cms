...
<script>
export default {
    props: ['retail_section'],
    data() {
        return {
            form: new Form(this.retail_section),
    ...
    created() {
        this.form.properties = JSON.parse(this.retail_section.properties)
    },

    watch: {
        'retail_section'() {
            this.form = new Form(this.retail_section);
            this.form.properties = JSON.parse(this.retail_section.properties)
        },
    ...
    methods: {
        ...
        updateAuctionSection() {
            this.isLoading = true;
            this.form.patch(`/retail-sections/${this.retail_section.id}`)
                .then(() => {
                    this.isLoading = false;
                    this.$snotify.async('Please wait', 'Processing...', () => new Promise((resolve, reject) => {
                        setTimeout(() => resolve({
                            title: 'Success!',
                            body: 'Retail Section Successfuly Updated!',
                            config: {
                                timeout: 2000,
