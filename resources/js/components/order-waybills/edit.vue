<template>
	<div ref="close" id="order-waybill-edit" class="modal" tabindex="-1" aria-hidden="true">
		<div class="modal-dialog modal-xl">
			<div class="modal-content">
				<!-- BEGIN: Modal Header -->
                <div class="modal-header bg-primary-1 text-theme-2">
                    <h2 class="font-medium text-base mr-auto">Update Waybill</h2>
                </div>
                <form class="">
                                ...
                                <div class="flex">
                                    ...
                                    <div class="w-full mr-1">
                                        <label class="form-label w-full flex flex-col sm:flex-row mt-3">
                                            Pickup Date
                                            <span class="sm:ml-auto mt-1 sm:mt-0 text-xs text-theme-6 "
                                    			v-if="form.errors.get('pickup_date')" v-html="form.errors.get('pickup_date')" />
                                        </label>
                                    ...
                                    <div class="w-full mr-1">
                                        <label class="form-label w-full flex flex-col sm:flex-row mt-3">
                                            Delivered Date
                                            <span class="sm:ml-auto mt-1 sm:mt-0 text-xs text-theme-6 "
                                    			v-if="form.errors.get('delivered_date')" v-html="form.errors.get('delivered_date')" />
                                        </label>
                                        <datepicker v-model="form.delivered_date"
                                            input-class="mb-2 border border-solid border-gray-300 px-2 py-2 rounded outline-none w-full flex-0 h-9">
                                        </datepicker>
                	...
                	<vue-snotify></vue-snotify>
						...
						<div class="modal-footer text-right">
							...
							<button @click.prevent="submit()" class="btn btn-primary w-20" :disabled="!form.pickup_address || !form.drop_off_address || !order_postings">Update</button>
						</div>
...
export default {
	...
	methods : {
		submit() {
			this.isLoading = true;
			this.form.patch(`/orders/shipmates-waybills/${this.order.id}`)
			.then((response)=>{
