<template>
    <div class="w-full">
        <table-template
            :link="'/events/' + $route.params.id + '/postings'"
            :fields="fields"
            :params="params"
            modalIdentifier="#add-posting"
            :additionalBtn="true"
            additionalBtnModalIdentifier="#event-posting-sequence"
            additionalBtnName="Posting Sequencing"
        >
            <template slot="label">
                <h4>List of Posting</h4>
            </template>
            <template slot="sequence" slot-scope="props">
                <span v-if="props.item.sequence == 0" class="font-medium">
                    N/A 
                </span>
                <span v-if="props.item.sequence != 0" class="font-medium">{{props.item.sequence}}</span>
            </template>
            <template slot="name" slot-scope="props">
                <span class="font-medium">{{ props.item.name }}</span>
            </template>
            <template slot="description" slot-scope="props">
                <span >{{props.item.description  | limitDescription(100)}}</span>
            </template>
            <template slot="extended_description" slot-scope="props">
                <span v-html="$options.filters.limitDescription(props.item.extended_description, 100)"></span>
            </template>
            ...
            <template slot="posting_id" slot-scope="props">
                ...
                <a
                    ...
                    href="javascript:;"
                    @click.prevent="
                    removePosting(props.item.posting_id)
                    "
        ...
        <create @created="resetSequence()"></create>
        <sequence :reset_sequence="reset_sequence"></sequence>
    </div>
</template>
<style>
.modal{
...
export default {
    components: { TableTemplate, Create, Sequence },
    data() {
        return {
            fields: {
                sequence: "Sequence",
    ...
    methods: {
        ...
        removePosting(id) {
            this.$modal
                .dialog({
                    message: "Are you sure you want to Remove this Posting?",
                    confirmButton: "Okay",
                    cancelButton: "Cancel",
                    title: "REmove",
                })
                .then((confirmed) => {
                    axios.patch(`/event-postings/${id}/remove`)
                        .then(() => {
                            this.$modal.success({
                                message: "Posting Successfully Removed",
                                title: "Success",
                            });
                            // Reload page
                            app.$emit("reload");
        ...
        resetSequence(){
            this.reset_sequence = true;
            setTimeout(()=>{
                this.reset_sequence = false;
            }, 100)
        }
    },
};
