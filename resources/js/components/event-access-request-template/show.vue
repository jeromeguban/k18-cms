...
<div>
	<div class="intro-y box p-5 mt-5">
        ...
        <div class="text-lg text-black font-semibold leading-loose">Show Details - Event</div>
        <p class="text-gray-700 leading-loose border-b border-gray-200 pb-2 mb-4">This page contains information about your Event details</p>
            ...
            <div class="flex gap-x-6">
                ...
                <div class="flex flex-col pb-3 gap-y-3 border-r border-gray-200">
                    <div class="flex leading-loose">
                        <div class="w-52 font-semibold pr-3">Event</div>
                        <div class="w-96 pr-3">{{ event.event_name }}</div>
                    </div>
                    <div class="flex leading-loose">
                        <div class="w-52 font-semibold pr-3">Email Type</div>
                        <div class="w-96 pr-3">{{ event.type }}</div>
                    </div>
					<div class="flex leading-loose">
                        <div class="w-52 font-semibold pr-3">Title</div>
                        <div class="w-96 pr-3">{{ event.title }}</div>
                    </div>
					<div class="flex leading-loose">
                        <div class="w-52 font-semibold pr-3">Body</div>
                        <div class="w-96 pr-3" v-html="event.body"></div>
                    </div>
                </div>
                <!-- 2nd column example -->
...
<script>
	export default {
		...
		methods: {
			show() {
				   axios.get('/event-access-request-templates/' + this.$route.params.id)
			        .then((response) => {
