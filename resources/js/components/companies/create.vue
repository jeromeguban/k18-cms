<template>
	<div>
		<div ref="close" id="companies-create" class="modal" tabindex="-1" aria-hidden="true">
							...
							<div class="input-form">
								<label class="form-label w-full flex flex-col sm:flex-row">
									...
									<span class="sm:ml-auto mt-1 sm:mt-0 text-xs text-theme-6 "
										v-if="form.errors.get('company_name')"
								...
								<label class="form-label w-full flex flex-col sm:flex-row mt-2">
									Company Code
									<span class="sm:ml-auto mt-1 sm:mt-0 text-xs text-theme-6 "
										v-if="form.errors.get('company_code')"
										v-html="form.errors.get('company_code')" />
								</label>
		...
		submit() {
			...
			.then(() => {
				 ...
				 this.$snotify.async('Please wait', 'Processing...', () => new Promise((resolve, reject) => {
					setTimeout(() => resolve({
						title: 'Success!',
						body: 'Company successfully created!',
						config: {
						timeout: 2000,
