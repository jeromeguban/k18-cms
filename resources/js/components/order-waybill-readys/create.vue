<template>
	<div ref="close" id="order-createwaybill" class="modal" tabindex="-1" aria-hidden="true">
		<div class="modal-dialog modal-xl">
			<div class="modal-content">
				<!-- BEGIN: Modal Header -->
                <div class="modal-header bg-primary-1 text-theme-2">
                    <h2 class="font-medium text-base mr-auto">Create New Order Waybill</h2>
                </div>
                <form class="">
                			...
                			<div class="flex" v-if="order">
                				<div class="w-full mr-1 box pb-2 pl-2 pt-2 pr-2 overflow-x-hidden overflow-visible" style="max-height: 200px;" >
									...
									<div class="flex justify-between border-b border-dashed border-gray-300 py-4">
										<span> Order Number : {{ order ? order.id : ' ' }} </span>
									</div>
									<div class="flex justify-between border-b border-dashed border-gray-300 py-4">
                					...
                					<div>
                						<table class="table table--sm">
                							...
                							<tr v-for="(order_posting, index) in order_postings">
                								...
                								<td> {{ order_posting.price | moneyFormat }} </td>
                							</tr>
                						</table>
                					</div>
                				</div>
									...
									<div class="w-full ml-1 box pb-2 pl-2 pt-2 pr-2">
	                					...
	                					<div>
	                						<table class="table table--sm">
	                							...
	                							<tr>
	                								<td> {{ fees.declared_value | moneyFormat }} </td>
	                								<td> {{ fees.shipmates_fee | moneyFormat }} </td>
	                								<td> {{ fees.shipping_fee | moneyFormat }} </td>
	                								<td> {{ fees.vat | moneyFormat }} </td>
	                								<td> {{ fees.valuation_fee | moneyFormat }} </td>
	                								<td> {{ fees.subtota | moneyFormatl }} </td>
	                								<td> {{ fees.total | moneyFormat }} </td>
	                							</tr>
	                						</table>
	                					</div>
	                				</div>
...
import Map from './../order-waybills/map';
export default {
	...
	methods : {
		...
		getOrderStoreContactDetails() {
			axios.get(`orders/${this.order.id}/store-contacts`)
			.then((response)=>{
