<template>
 <div ref="close" id="store-address-edit" class="modal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <!-- BEGIN: Modal Header -->
            <div class="modal-header bg-primary-1 text-theme-2">
                <h2 class="font-medium text-base mr-auto">Edit Store Address</h2>
            </div>
            <form class="">
...
export default {
	...
	data() {
	 	...
	 	created() {
            this.address = new Form(this.store_address);
            this.province = this.store_address.province
			this.getCountries();
            this.getProvinces();
            this.getBarangays();
            this.getZipcodes();
	...
	methods: {
            submit() {
              this.isLoading = true;
              this.address.patch(`/update/${this.store_address.address_id}/store-address`)
                .then(() => {
                    this.isLoading = false;
                    this.$snotify.async('Please wait', 'Processing...', () => new Promise((resolve, reject) => {
                        setTimeout(() => resolve({
                            title: 'Success!',
                            body: 'Store Address Successfully Updated!',
                            config: {
                            timeout: 2000,
           ...
           myLocation() {
            ...
            currentSetLocation() {
                this.getAddressFrom(
                     this.store_address.latitude,
                     this.store_address.longitude
                );
