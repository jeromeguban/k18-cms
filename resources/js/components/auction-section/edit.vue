<template>
    <div ref="close" id="large-slide-over-size-preview" class="modal modal-slide-over" tabindex="-1" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <form>
                    ...
                    <div class="modal-footer text-right">
                        ...
                        <button @click.prevent="updateAuctionSection()" class="btn btn-primary w-20">Update</button>
                    </div>
                </form>
            </div>
        </div>
...
<script>
export default {
    props: ['auction_section'],
    data() {
        return {
            form: new Form(this.auction_section),
    ...
    created() {
        this.form.properties = JSON.parse(this.auction_section.properties)
    },

    watch: {
        'auction_section'() {
            this.form = new Form(this.auction_section);
            this.form.properties = JSON.parse(this.auction_section.properties)
        },
    ...
    methods: {
        ...
        updateAuctionSection() {
            this.isLoading = true;
            this.form.patch(`/auction-sections/${this.auction_section.id}`)
                .then(() => {
                    this.isLoading = false;
                    this.$snotify.async('Please wait', 'Processing...', () => new Promise((resolve, reject) => {
                        setTimeout(() => resolve({
                            title: 'Success!',
                            body: 'Auction Section Successfuly Updated!',
                            config: {
                                timeout: 2000,
        ...
        uploadImage(index) {
            console.log(index);
            this.form_upload.post(`section-images`)
                .then(({
                    data
                }) => {
