<template>
	<div>
		<div class="intro-y flex flex-col sm:flex-row items-center mt-8">
			<h2 class="text-lg font-medium mr-auto">Customize Sidebar Section</h2>
			<div class="w-full sm:w-auto flex mt-4 sm:mt-0">
				<div class="flex items-center justify-end">
					...
					<a href="#" @click.prevent="" data-toggle="modal" data-target="#sidebar-create"
			...
			<div class="px-5 sm:px-10 py-5 sm:py-10">
				<div class="w-full">
					<draggable v-model="navigations" @change="updateSequenced"
						...
						<div v-for="(navigation, index) in navigations" :key="navigation.id"
							class="intro-x justify-center">
							<div class="justify-center">
								<div
									class="flex flex-col w-full border-theme-13 border-solid shadow-md rounded-b pb-2 mt-6">
									<div
										...
										<span class="flex">
											<a class="flex" href="#" data-toggle="modal" data-target="#sidebar-edit"
									...
									<div class="flex flex-row justify-center">
										<div class="flex">
											<draggable v-if="navigation.property.length" v-model="navigation.property"
												...
												style="max-height: 150px; min-height: 150px; max-width:420px; min-width: 420px;">
												<div class="flex flex-col"
													v-for="(property, index) in navigation.property" :key="index">
													<div class="flex py-2">
														...
														<div class="flex justify-between w-full">
															...
															<div class="flex items-center justify-center text-2xs mr-4">
																<i
																	:class="'fa fa-' + property.icon + ' vfa-icon-preview'"></i>
															</div>
															...
															<div class="flex cursor-pointer">
																<a class="flex text-theme-1" href="#"
																	data-toggle="modal"
																	data-target="#sidebar-properties-edit"
									...
									<div class="flex flex-row justify-center">
										<div class="flex w-1/2 px-5 items-center justify-center">
											<a href="#" data-toggle="modal" data-target="#add-column"
												@click.prevent="setNavigation(navigation)"
												class="zoom-in flex flex-row justify-center intro-x w-1/2 text-center rounded-md py-3 border border-dotted border-slate-400 dark:border-darkmode-300 text-slate-500">
												<div class="flex">
													<svg data-v-0fe7c018="" xmlns="http://www.w3.org/2000/svg"
														viewBox="0 0 512 512" class="fill-current w-4 h-4 mr-2">
														<path data-v-0fe7c018=""
															d="M384 250v12c0 6.6-5.4 12-12 12h-98v98c0 6.6-5.4 12-12 12h-12c-6.6 0-12-5.4-12-12v-98h-98c-6.6 0-12-5.4-12-12v-12c0-6.6 5.4-12 12-12h98v-98c0-6.6 5.4-12 12-12h12c6.6 0 12 5.4 12 12v98h98c6.6 0 12 5.4 12 12zm120 6c0 137-111 248-248 248S8 393 8 256 119 8 256 8s248 111 248 248zm-32 0c0-119.9-97.3-216-216-216-119.9 0-216 97.3-216 216 0 119.9 97.3 216 216 216 119.9 0 216-97.3 216-216z">
														</path>
													</svg>
												</div>
...
import EditProperty from "./../sidebar/edit-property";
export default {
	components: {
		Create,
		Edit,
	...
	methods: {
		getHeader() {
			axios.get(`/navigations?type=sidebar`)
				.then((response) => {
			...
			this.$modal.dialog({
				...
				.then(confirmed => {
					axios.delete(`navigations/${id}`)
						.then(() => {
							this.$modal.success({
								message: 'You successfully deleted a Sidebar',
								title: 'Deleted'
							});
							// Reload page
							app.$emit('reload');
		...
		updateProperty(navigation) {
			...
			axios.post(`navigations/properties-sequence`, {
				...
				this.$snotify.async('Please wait', 'Processing...', () => new Promise((resolve, reject) => {
					setTimeout(() => resolve({
						title: 'Success!',
						body: 'Sidebar successfully updated!',
						config: {
							timeout: 2000,
							showProgressBar: true,
							closeOnClick: false,
