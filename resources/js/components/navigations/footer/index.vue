<template>
	<div>
		<div class="intro-y flex flex-col sm:flex-row items-center mt-8">
			<h2 class="text-lg font-medium mr-auto">Customize Footer Section</h2>
			<div class="w-full sm:w-auto flex mt-4 sm:mt-0">
				<div class="flex items-center justify-end">
					...
					<a href="#" @click.prevent="" data-toggle="modal" data-target="#footer-create"
							...
							<div class="">
								<div class="flex flex-col w-full border-theme-13 border-solid shadow-md rounded-b pb-2">
									...
									<div
										...
										<span class="flex">
											<a class="flex" href="#" data-toggle="modal" data-target="#footer-edit"
									...
									<draggable v-model="navigation.property" group="property"
										@change="updateProperty(navigation)"
										:options="{ handle: '.item', animation: 150, scrollSensitivity: 200, forceFallback: true }"
										class="overflow-y-scroll overflow-x-hidden w-full px-2 lg:px-4"
										style="max-height: 250px; min-height: 250px; max-width:320px; min-width: 320px;">
												...
												<div class="flex justify-between w-full">
													...
													<div class="flex cursor-pointer">
														<a class="flex text-theme-1" href="#" data-toggle="modal"
															data-target="#footer-properties-edit"
...
import EditProperty from "./../footer/edit-property";
export default {
	components: {
		Create,
		Edit,
	...
	methods: {
		getFooters() {
			axios.get(`/navigations?type=footer`)
				.then((response) => {
		...
		deleteNavigation(id) {
			this.$modal.dialog({
				message: 'Are you sure you want to delete this Footer?',
				confirmButton: 'Okay',
				cancelButton: 'Cancel',
				title: 'Delete'
			})
				.then(confirmed => {
					axios.delete(`navigations/${id}`)
						.then(() => {
							this.$modal.success({
								message: 'You successfully deleted a Footer',
								title: 'Deleted'
							});
							// Reload page
							app.$emit('reload');
		...
		updateProperty(navigation) {
			...
			axios.post(`navigations/properties-sequence`, {
				...
				this.$snotify.async('Please wait', 'Processing...', () => new Promise((resolve, reject) => {
					setTimeout(() => resolve({
						title: 'Success!',
						body: 'Footer successfully updated!',
						config: {
							timeout: 2000,
							showProgressBar: true,
							closeOnClick: false,
