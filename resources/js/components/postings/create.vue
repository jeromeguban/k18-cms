<template>
    <div ref="close" id="postings-create" class="modal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">

                <!-- BEGIN: Modal Header -->
                <div class="modal-header bg-primary-1 text-theme-2">
                    <h2 class="font-medium text-base mr-auto">Create New Posting</h2>
                    <!-- <button class="btn btn-outline-secondary hidden sm:flex">
                    <i data-feather="file" class="w-4 h-4 mr-2"></i> Download Docs
                </button> -->
                </div>

                <form class="" @keydown.enter.prevent="" @submit.prevent="submit">
                            ...
                            <div class="flex flex-col sm:flex-row">
                                <div class="w-1/2 ml-1 mr-1">
                                    <label class="form-label w-full flex flex-col sm:flex-row mt-4 ml-1">
                                        Categories
                                        <span class="sm:ml-auto mt-1 sm:mt-0 text-xs text-theme-6 "
                                            v-if="form.errors.get('categories')" v-html="form.errors.get('categories')" />
                                    </label>
                                ...
                                <div class="w-1/2 ml-1 mr-1">
                                    <label class="form-label w-full flex flex-col sm:flex-row mt-4">
                                        Brands
                                        <span class="sm:ml-auto mt-1 sm:mt-0 text-xs text-theme-6 "
                                            v-if="form.errors.get('brands')" v-html="form.errors.get('brands')" />
                                    </label>
...
export default {
    ...
    methods: {
        submit() {
            this.isLoading = true;
            this.form.post('postings')
                .then(() => {
                    this.isLoading = false;
                    this.$snotify.async('Please wait', 'Processing...', () => new Promise((resolve, reject) => {
                        setTimeout(() => resolve({
                            title: 'Success!',
                            body: 'Posting Successfuly Created!',
                            config: {
                                timeout: 2000,
